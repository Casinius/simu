...
public:
    void update(Time_Unit dt) {
        ...
        if (current_rpm < 0.0) current_rpm = 0.0;
    }
...
public:
    void update(RSpeed_Unit engine_av, RSpeed_Unit trans_input_av, Force_Unit engine_torque, Force_Unit trans_load_torque, Time_Unit dt) {
        ...
        if (!locked) {
            // 动态摩擦：可引入速度相关的摩擦系数下降
            max_transfer = friction_capacity * (1.0 - 0.1 * std::min(1.0, std::abs(delta_av) / 100.0));
        }
        ...
        if (locked) {
            ...
            if (std::abs(delta_av) < slip_threshold_av && std::abs(desired_torque) < max_transfer)
                locked = true;
        }
...
public:
    ...
    void update(Time_Unit dt, ClutchComponent& clutch, double engine_rpm, double wheelspeed) {
        if (is_shifting) {
            shift_timer -= dt;
            if (shift_timer <= 0.0) {
                // 换档完成，重新接合离合器
...
public:
    void distribute(Force_Unit input_torque, double left_speed, double right_speed,
                    Force_Unit& left_torque, Force_Unit& right_torquem,Time_Unit dt) {
        if (type == LOCKED) {
